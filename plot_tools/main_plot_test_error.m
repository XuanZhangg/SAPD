...
load(name)
options.x_axis = [1:length(test_error)];
options.color = color{iter};
options.alg = 'OGDA';
p1 = plot_areaerrorbar(test_error, options);
labels{iter} = 'S-OGDA';
%% SMD
% iter = iter + 1;
% name = ['SMD',' batchsize=', num2str(batchsize), ' sim_num =',num2str(sim_num),' data_id =', num2str(data_id), '.mat'];
% load(name)
% options.x_axis = [1:length(test_error_bar)];
% options.color = color{iter};
% options.alg = 'SMD';
% p2 = plot_areaerrorbar(test_error_bar, options);
% labels{iter} = 'SMD';
%% SMP
iter = iter + 1;
...
options.color = color{iter};
options.alg = 'SMP';
p3 = plot_areaerrorbar(test_error, options);
labels{iter} = 'SMP';
%% SAPD
...
for rho = rho_set{data_id}
    for batchsize_x = [batchsize]
        for batchsize_y = [1]
            ...
            %name = ['SAPD rho=',num2str(para.rho),' batchsize_x=', num2str(batchsize_x),' batchsize_y=', num2str(batchsize_y), ' sim_num =',num2str(sim_num),' data_id =', num2str(data_id), '.mat'];
            load(name)
            options.x_axis = [1:length(test_error(1,:))];
            options.color = color{iter};
            %APD_figure{APD_count} = plot_areaerrorbar(test_error([1:9,11:12,14,16:18,20:38,40:48,50],:), options);%pick for batchsize = 1
            APD_figure{APD_count} = plot_areaerrorbar(test_error_bar, options);
            %APD_figure{APD_count} = plot(mean(test_error_bar), 'color', options.color,'LineWidth', 3);
...
% create ylabel
ylabel({'Test Error'},'Interpreter','tex','fontsize', outersize);
...
xlim([0 Leng])
%% Save the figure
figure_name_1 = ['./figure/',dataname{data_id}, '_test_error_batchsize_', num2str(batchsize_x),'.fig'];
figure_name_2 = ['./figure/',dataname{data_id}, '_test_error_batchsize_', num2str(batchsize_x),'.eps'];
